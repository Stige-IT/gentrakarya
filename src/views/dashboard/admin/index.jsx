...
const Admin = () => {
    ...
    useEffect(() => {
        ...
        if (accessToken === null || accessToken === '') {
            ...
            getAdmin(accessToken).then((response) => {
    ...
    const handleSubmitAdd = async (e) => {
        ...
        setLoadingAdd(true)
        try {
            await postAdmin(accessToken, name, email, genderId, password, passwordConfirmation);
            ...
            setShowModalAdd(false)
            setLoadingAdd(false)
            alert('Maaf tambah admin tidak berhasil, silahkan periksa kembali formulir yang telah disediakan..!')
        }
    }

    const handleSearch = (e) => {
        e.preventDefault()
        searchAdmin(accessToken, keyword).then((response) => {
            console.log(response.data.data);
            if (response && response.status === 200) {
                setData(response.data.data)
                setTotalData(response.data.total)
    ...
    const handleDeleteAdmin = (adminId) => {
        deleteAdmin(accessToken, adminId).then((response) => {
    ...
    return (
        <>
            <Helmet>
                <title>Admin - Gentra Karya</title>
            </Helmet>
            <Layout>
                <Header>
                    <p>Data Admin <span className="text-xl">({totalData})</span></p>
                    <button type="button"
                                ...
                                <tr className={`hover:bg-gray-300 ${number % 2 === 0 ? 'bg-gray-200' : ''}`}>
                                    ...
                                    <td className="px-3 py-2 ">
                                        {item?.image === null ?
                                            ...
                                            <img src={BaseURL + item?.image} alt="Foto Profile Admin - Gentra Karya" className="w-14 aspect-square rounded-full object-cover mx-auto" />
                                        }
                                    </td>
                                    ...
                                    <td className="px-3 py-2 w-20 h-full transition-colors">
                                        <button type="button" className=" my-2 btn btn-solid-warning">
                                            ...
                                            <div className="dropdown-menu w-72">
                                                <div className="p-5 flex flex-col items-center justify-center space-y-3 ">
                                                    ...
                                                    <div className="flex items-center space-x-3 text-sm text-white">
                                                        ...
                                                        <label onClick={() => handleDeleteAdmin(item?.id)} className="btn btn-error w-32">
                                                            Ya
                                                        </label>
                                                    </div>
                                                </div>
                ...
                <form onSubmit={handleSubmitAdd}>
                    ...
                    <div className="modal-content flex flex-col gap-5 max-w-3xl">
                        ...
                        <div className="space-y-2 text-sm lg:w-96">
                            ...
                            <div className="my-1">
                                <label htmlFor="" className="font-semibold lg:w-44">Pilih Jenis Kelamin</label>
                                <select value={genderId} onChange={(e) => setGenderId(e.target.value)} className="w-full px-3 py-2 outline-none rounded-md border-2 text-black focus:border-custom-yellow">
                                    {gender?.map((item) => (
                            ...
                            <TemplateInput
                                typeInput={showPassword === true ? 'text' : 'password'}
                                name="Password"
                                value={password}
                            ...
                            <div className="flex items-center space-x-3">
                                <button type="button" onClick={handleShowPassword} className={` flex items-center space-x-3 hover:text-blue-500 transition-colors ${showPassword === true ? 'text-blue-500' : 'text-black'}`}>
                                    <FontAwesomeIcon icon={faEye}></FontAwesomeIcon>
                                    <p>Perlihatkan password</p>
                                </button>
                            </div>
                        </div>
