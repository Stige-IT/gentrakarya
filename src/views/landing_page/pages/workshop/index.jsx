...
import {
  getWorkshopGeneral,
  searchWorkshopGeneral,
} from "../../../../services/workshop_service";
...
const Workshop = () => {
  ...
  const handleProvince = (e) => {
    ...
    getRegency(provinceId).then((response) => {
      ...
      getWorkshopGeneral(provinceId, regencyId, districtId, villageId).then(
        (response) => {
          if (response && response.status === 200) {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    searchWorkshopGeneral(keyword).then((response) => {
      if (response && response.status === 200) {
        setData(response.data.data);
        setTotalData(response.data.meta.total);
        setProvinceId(0);
  ...
  const fetchDataWorkshop = async (provinceId, regencyId, districtId, villageId, page) => {
    try {
      const response = await getWorkshopGeneral(provinceId, regencyId, districtId, villageId, page)
      if (response && response.status === 200) {
        setData(response.data.data);
        setTotalData(response.data.meta.total);
  ...
  return (
    <>
      <Helmet>
        <title>Workshop - Gentra Karya</title>
      </Helmet>
